        ...
        <form onSubmit={handleCreate} className="bg-white rounded-xl border border-surface-200 p-5 space-y-4 animate-slideDown">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={form.siret} onChange={(e) => { setForm({ ...form, siret: e.target.value.replace(/\D/g, '').slice(0, 14) }); if (errors.siret) setErrors((p) => { const n = { ...p }; delete n.siret; return n }) }} className="w-full rounded-lg border border-surface-300 px-4 py-2 text-sm outline-none" maxLength={14} />
              {errors.siret && <p className="mt-1 text-xs text-red-600">{errors.siret}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-surface-700 mb-1">Email</label>
              <input type="email" value={form.email} onChange={(e) => { setForm({ ...form, email: e.target.value }); if (errors.email) setErrors((p) => { const n = { ...p }; delete n.email; return n }) }} className="w-full rounded-lg border border-surface-300 px-4 py-2 text-sm outline-none" />
